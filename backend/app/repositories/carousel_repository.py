...
class CarouselRepository(BaseRepository):
    ...
    def delete_carousel(self, carousel_id: int) -> bool:
        # Delete all media first (cascade handled by ORM relationship too)
        for media in self.get_carousel_media(carousel_id):
    ...
    def get_carousel_media(self, carousel_id: int) -> List[Dict[str, Any]]:
        ...
        active = [m for m in media if m.get("is_active", True)]
        return sorted(active, key=lambda x: x.get("display_order", 999))
    ...
    def get_all_carousels_with_media(self) -> List[Dict[str, Any]]:
        ...
        for c in carousels:
            c["media"] = self.get_carousel_media(c["id"])
        return carousels
    ...
    def get_media_by_type(self, media_type: str) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            if m.get("media_type", "").lower() == media_type.lower() and m.get("is_active", True)
        ]
